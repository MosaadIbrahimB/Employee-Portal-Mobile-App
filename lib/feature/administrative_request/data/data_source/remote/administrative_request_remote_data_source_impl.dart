...
class AdministrativeRequestRemoteDataSourceImpl
    implements AdministrativeRequestRemoteDataSource {
  final DioConsumer apiService;

  AdministrativeRequestRemoteDataSourceImpl({required this.apiService});

  @override
  Future<List<AdministrativeRequestTypeModel>>
  getAdministrativeRequestType() async {
  ...
  getEmployeeAdministrativeRequest() async {
    ...
    final responsePending = await apiService.get(
      endPoint: EndPoint.getEmployeeAdminFalse,
    );
    final listPending =
  ...
  getReviewerAdministrativeRequest() async {
    final responseReviewed = await apiService.get(
      endPoint: EndPoint.getReviewerAdminTrue,
    );
    final listReviewed =
        responseReviewed.data
            .map<ResponseAdministrativeModel>(
